...
<script>
    ...
    // @ts-nocheck
    export let title;
    export let isUpdateEmployeeModalopen = false;
    ...
    function setUpdateValues(){
        if(currentEmployee === undefined)
        {
            currentEmployee =[
                ...
                {empCivStatus:"NA"},
                {empCivService:"NA"},
                {empWorkExperience:"NA"},
                {empCharityWork:"NA"},
                {empLearningAndDevelopment:"NA"},
                {emp_file:"NA"},
            ]
        }
        _id = currentEmployee._id;
        emp_picture = currentEmployee.emp_picture;
        empNo = currentEmployee.empNo;
        empScanNo = currentEmployee.empScanNo;
        empFullName = currentEmployee.empFullName;
        emp_div = currentEmployee.emp_div;
        emp_email = currentEmployee.emp_email;
        empDateofBirth = currentEmployee.empDateofBirth;
        empPlaceofBirth = currentEmployee.empPlaceofBirth;
        empSex = currentEmployee.empSex;
        empCivStatus = currentEmployee.empCivStatus;
        empCivService = currentEmployee.empCivService;
        empWorkExperience = currentEmployee.empWorkExperience;
        empCharityWork = currentEmployee.empCharityWork;
        empLearningAndDevelopment = currentEmployee.empLearningAndDevelopment;
    ...
    async function handleSubmit(event){
        ...
        const response = await fetch('api/admin/Employee/update', {
            method: 'POST',
            headers: {
...
<div class="fixed z-10 inset-0 overflow-y-auto {isUpdateEmployeeModalopen ? 'block' : 'hidden'}">
