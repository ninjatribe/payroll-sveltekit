<script>
	...
	export let currentsubLoan;
	export let loadSubloan = () => {};

	const subloan = currentsubLoan;
	const _id = subloan._id;

	const handleCloseModal = () => (isDeleteDataOpen = false);

	async function handleConfirmDelete(event) {
		event?.preventDefault();
		const response = await fetch('/api/admin/subloan/delete', {
			method: 'POST',
			headers: {
				...
				<div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
					...
					<div class="p-6 text-center">
						<svg
							...
							/></svg
						>
						<h2 class="text-lg font-normal text-gray-800 dark:text-gray-400">
							{currentsubLoan.code}
						</h2>
						<h2 class="mb-2 text-lg font-normal text-gray-800 dark:text-gray-400">
							{currentsubLoan.description}
						</h2>
