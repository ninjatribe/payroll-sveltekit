<script>
	...
	function editValues() {
		...
		postionTitle = currentPosition.postionTitle;
		postionState = currentPosition.postionState;
		salaryGrade = currentPosition.salaryGrade;
		payGradeSteps = currentPosition.payGradeSteps;
		payBasis = currentPosition.payBasis;
		vacancy = currentPosition.vacancy;
		plantillaItemNo = currentPosition.plantillaItemNo;
		remarks = currentPosition.remarks;
	}
	...
	async function submitData(event) {
		console.log(code);
		console.log(postionTitle);
		console.log(postionState);
		console.log(salaryGrade);
		console.log(payGradeSteps);
		console.log(payBasis);
		console.log(vacancy);
		console.log(plantillaItemNo);
		console.log(remarks);
		event?.preventDefault();
		const response = await fetch('/api/admin/positions/update', {
			method: 'POST',
			headers: {
	...
	<div class="flex items-center justify-center min-h-screen px-4">
		...
		>
			<div class="bg-gray-100 px-4 py-3 border-b">
				<h4 class="text-lg font-medium text-gray-600">Edit Position</h4>
			</div>
			<div class="px-4 py-5 sm:p-6">
				...
				<div class="mb-4">
					...
					<select
						...
						bind:value={remarks}
					>
						<option value="Biometrix Break">Biometrix Break</option>
						<option value="DTR">DTR</option>
						<option value="Leave Recommendation">Leave Recommendation</option>
