<script>
    ...
    export let isDeleteDataOpen = false;
    ...
    const handleCloseModal = () => isDeleteDataOpen = false;
    

    async function handleConfirmDelete(event)
    {
		event?.preventDefault();
		const response = await fetch('/api/admin/leave/delete', {
			method: 'POST',
			headers: {
...
<div class="fixed z-10 inset-0 overflow-y-auto {isDeleteDataOpen? 'block': 'hidden'}">
            ...
            <div class="relative w-full h-full max-w-md md:h-auto">
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <div class="p-6 text-center">
                        ...
                        <h2 class="text-lg font-normal text-gray-800 dark:text-gray-400">{currentLeave.code}</h2>
                        <h2 class="mb-2 text-lg font-normal text-gray-800 dark:text-gray-400">{currentLeave.description}</h2>
                        <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">Are you sure you want to delete this division?</h3>
                        <Button
