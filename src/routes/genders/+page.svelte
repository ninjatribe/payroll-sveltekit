<script>
	...
	function currentGenderExist() {
		if (currentGender === undefined || !items.includes(currentGender)) {
			log.error('Selected gender does not exist in items fetch from database!');
			return false;
		}
		if (gender.code === '' || gender.description === '') {
			return false;
		}
		return true;
	}
