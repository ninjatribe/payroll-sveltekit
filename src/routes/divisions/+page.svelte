	...
	$: {
		...
		paginatedItems = search
			? items.filter((division) => {
					return status !== 'all'
						? (division.code.match(RegExp(search, 'gi')) ||
								division.description.match(RegExp(search, 'gi'))) &&
								division.isActive === (status === 'active')
						: division.code.match(RegExp(search, 'gi')) ||
								division.description.match(RegExp(search, 'gi'));
			  })
			: items.filter((division) => {
					return status !== 'all' ? division.isActive === (status === 'active') : items;
			  });
...
<div class="border-2 border-gray-100 rounded-lg h-auto dark:border-gray-700 mt-12">
	<div class="flex flex-col justify-center border-b h-fit rounded bg-blue-600 dark:bg-gray-800">
		<div class="flex flex-col px-5 justify-center py-4">
			<span class="text-xl font-semibold" style="color:white">Divisions</span>
			<span class="text-m" style="color:white">Manage Divisions</span>
		</div>
